...
namespace SystAnalys_lr1
{
    class GA
    {
        ...
        public GA (List<Vertex> vertexCollection, List<Edge> edgeCollection, int[] steinerVertexCollection, /*List<Point2D> steinerVertexCollection, List<Edge> steinerEdgeCollection,*/ int populationSize, int generationSize, double maxCost, double limitCostRate, double mutateRate, int CarCount)
        {
        ...
        public void Go()
        {
            ...
            {
                if ((int)_betterChromosome.Fitness == 100)
                {
                    break;
                }
                CompareChromosome(population);//сравниваем первый элемент с совсеми хромосомами в популяции, находим наименьшую?
                ...
                Log.WriteLog("Iteration "+ iteration+": "+ Math.Round(min));// выводим номер итерации и минимальную эффективность округленную
                ...
                {
                    splitpopulation +=(Math.Round(chromosome.Fitness).ToString())+" ";// cоздаем строку с эффективностью для каждой хромосомы в популяции
        ...
        private void CompareChromosome(List<Chromosome> population)
        {
            foreach (var chromosome in population)
            {
                if (chromosome.Fitness >= _betterChromosome.Fitness)
                {
